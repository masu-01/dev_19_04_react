...
export const News3CardDemo = ({text, img, timestamp}) => {
 React.memo(function News3Card() {
  const styles = useStyles();
  const mediaStyles = useCoverCardMediaStyles();
//   const PostCard = ({text, img, timestamp});
  return (
    <>
      <NoSsr>
      ...
      <Card className={styles.card}>
        ...
        >
          ...
          <Info position={'middle'} useStyles={useNewsInfoStyles}>
            ...
            <InfoSubtitle>{new Date(timestamp?.toDate()).toLocaleString()}</InfoSubtitle>
          </Info>
        </Row>
